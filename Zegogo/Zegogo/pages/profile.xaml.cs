	...
	public partial class profile : ContentPage
	{
		...
		public async void getUser()
		{
			try
			{
				...
				userProf = JsonConvert.DeserializeObject<user>(json5);
				imgAvatar.Source = userProf.big_avatar;
			}
			catch(Exception ex)
			{
				await DisplayAlert("Get user", ex.Message, "Ok");
			}
		...
		public async void getUserOffers()
		{
			try
			{
				...
				HttpResponseMessage response2 = await client2.GetAsync(sb2.ToString());
				var json5 = await response2.Content.ReadAsStringAsync();
				await DisplayAlert("Get user offers", json5, "Ok");
			}
			catch (Exception ex)
			{
		...
		private async void Button_Clicked(object sender, EventArgs e)
		{
			try
			{
				...
				HttpClient client2 = new HttpClient();
				StringBuilder sb2 = new StringBuilder($@"https://zegogo.com/web/api/v1/user/logout?access_token={oauthToken}&signature={md5temp2}");
				HttpResponseMessage response2 = await client2.GetAsync(sb2.ToString());
				var json5 = await response2.Content.ReadAsStringAsync();
				await SecureStorage.SetAsync("is login", "no");
				TabbedPage tempTP = Parent as TabbedPage;
				(Parent as TabbedPage).Children.Clear();
				tempTP.Children.Add(new advertisement());
				tempTP.Children.Add(new loginPage { IconImageSource = "tab2"});
				tempTP.Children.Add(new loginPage { IconImageSource = "tab3"});
				tempTP.Children.Add(new loginPage { IconImageSource = "tab4"});
				tempTP.Children.Add(new loginPage { IconImageSource = "tab5"});

			}
			catch (Exception ex)
			{
				await DisplayAlert("Logout", ex.Message, "Ok");
			}
		}
		...
		private async void TapGestureRecognizer_Tapped(object sender, EventArgs e)
		{
			try
			{
				...
				Debug.WriteLine(sb2);
				await Navigation.PushAsync(new myOffers(result), false);
			}
			catch (Exception ex)
			{
				await DisplayAlert("My offers", ex.Message, "Ok");
			}
		...
		{
			(Parent as TabbedPage).CurrentPage = (Parent as TabbedPage).Children[2];
		}
	}
}