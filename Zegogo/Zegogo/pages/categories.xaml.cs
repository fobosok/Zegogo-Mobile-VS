...
namespace Zegogo.pages
{
	...
	public partial class categories : ContentPage , IAnimationPage
	{
		...
		public categories(ListCategory listCategory)
		{
			InitializeComponent();
		...
		private async void TapGestureRecognizer_Tapped(object sender, EventArgs e)
		{
			...
			try
			{
				string catId = (((sender as Frame).Content as StackLayout).Children[3] as Label).Text;
				string title = (((sender as Frame).Content as StackLayout).Children[1] as Label).Text;
				(sender as Frame).BackgroundColor = Color.FromHex("#FFEBCD");
				await (sender as Frame).ScaleTo(1.1, 50);
				await (sender as Frame).ScaleTo(1, 250);
				(sender as Frame).BackgroundColor = Color.White;
				if ((((sender as Frame).Content as StackLayout).Children[2] as Image).IsVisible == false)
				{
					MessagingCenter.Send<string>(catId, "LabelChange");
				...
				else
				{
					...
					foreach (var item in ListCategory.categories)
					{
						...
						await Navigation.PushAsync(new subcategory(ListCategory, title, catId));
				}
			}
			catch (Exception exp)
			{
		...
		private async void TapGestureRecognizer_Tapped_1(object sender, EventArgs e)
		{
			...
			(sender as Frame).BackgroundColor = Color.White;
				...
				MessagingCenter.Send<string>("all", "LabelChange");
